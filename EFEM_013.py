...
class EfemApp:
    def __init__(self, root):
        self.root = root
        self.root.title("EFEM 測試程式 v1.4 (加入 EFEM 控制)")
        ...
        self.ip_entry = ttk.Entry(top_frame, width=15); self.ip_entry.pack(side=LEFT, padx=5); self.ip_entry.insert(0, DEFAULT_EFEM_IP)
        ttk.Label(top_frame, text="Port:").pack(side=LEFT, padx=5)
        self.port_entry = ttk.Entry(top_frame, width=7); self.port_entry.pack(side=LEFT, padx=5); self.port_entry.insert(0, str(DEFAULT_EFEM_PORT))
        self.connect_button = ttk.Button(top_frame, text="連線", command=self.connect); self.connect_button.pack(side=LEFT, padx=5)
        self.disconnect_button = ttk.Button(top_frame, text="斷線", command=self.disconnect, state=tk.DISABLED); self.disconnect_button.pack(side=LEFT, padx=5)
        self.connection_status_label = ttk.Label(top_frame, text="已斷線", foreground="red", font=("Arial", 10, "bold")); self.connection_status_label.pack(side=LEFT, padx=10)
        ...
        left_panel.pack(side=LEFT, fill=Y, padx=(0, 5), anchor='nw') # anchor='nw' 確保從左上角開始填充
        ...
        # --- 填充左側面板 ---
        self._create_efem_panel(left_panel)      # **新增 EFEM 面板**
    ...
    def _create_efem_panel(self, parent):
        ...
        efem_actions = [
            "Home,EFEM", "GetStatus,EFEM", "GetVersion,EFEM",
            "Remote,EFEM", "Local,EFEM",
            ...
            "SignalTower,EFEM,All,Off",
            "SetBuzzer,EFEM,1,On", "SetBuzzer,EFEM,1,Off", "SetBuzzer,EFEM,1,Flash",
            # 更多指令可加在此處，例如 SetTowerLampInterval, MusicBuzzer 等，可能需要額外參數輸入
        ]
        ttk.Label(action_frame, text="選擇 EFEM 動作:").pack(side=LEFT, padx=5)
        self.efem_action_combo = ttk.Combobox(action_frame, textvariable=self.efem_action_var, values=efem_actions, width=35, state="readonly")
        self.efem_action_combo.pack(side=LEFT, padx=5)
        if efem_actions: self.efem_action_combo.current(0)

        # 按鈕行
        button_frame = ttk.Frame(frame)
        button_frame.pack(fill=X, pady=5)
        ttk.Button(button_frame, text="加入", width=8, command=self.add_efem_action_to_sequence).pack(side=LEFT, padx=5)
        ttk.Button(button_frame, text="插入", width=8, command=self.insert_efem_action_to_sequence).pack(side=LEFT, padx=5)
        ttk.Button(button_frame, text="執行", width=8, command=self.execute_efem_action_directly).pack(side=LEFT, padx=5)
    ...
    def _create_aligner_panel(self, parent):
        ...
        frame.pack(pady=5, padx=5, fill=X)
        # ... (內容與 V2 版本相同，但 SetWaferSize 改用 Entry) ...
        row1 = ttk.Frame(frame); row1.pack(fill=X, pady=2)
        ...
        self.al_size_entry = ttk.Entry(row3, width=5) # **用於 SetWaferSize**
    ...
    def _create_sequence_panel(self, parent):
        ...
        ttk.Button(mid_control_frame, text="加入 Wait", command=self.add_wait_to_sequence).pack(side=LEFT, padx=5) # Wait 加入按鈕
        ttk.Button(mid_control_frame, text="插入 Wait", command=lambda: self.add_wait_to_sequence(insert=True)).pack(side=LEFT, padx=5) # Wait 插入按鈕
    ...
    # --- 獲取指令字串的輔助函數 (與 V2 版本相同，已加入 Robot Speed, Aligner Size) ---
    ...
    def execute_efem_action_directly(self):
        ...
        if command:
            ...
            self.log_message("錯誤：請選擇一個 EFEM 動作。\n")
