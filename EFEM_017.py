...
# --- 模擬器元件位置定義 ---
# 注意：這些位置是相對於 Simulation Widget 的，不受主視窗大小影響
SIM_LP1_RECT = QRect(20, 100, 80, 150)
SIM_ROBOT_RECT = QRect(150, 120, 60, 60)
SIM_ALIGNER_RECT = QRect(260, 50, 60, 60)
SIM_STAGE1_RECT = QRect(260, 190, 60, 60)
SIM_BUFFER1_RECT = QRect(370, 120, 60, 60) # 假設有 Buffer
SIM_WAFER_SIZE = 10 # Wafer 在模擬器中的大小

# --- 模擬指令顯示視窗 ---
class SimulationLogWindow(QWidget):
    """一個獨立的小視窗，用於顯示模擬指令"""
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("模擬指令記錄")
        # 調整模擬指令視窗的初始位置和大小
        self.setGeometry(860, 50, 350, 250) # 放在主視窗旁邊，尺寸稍小

        layout = QVBoxLayout(self)
        self.log_edit = QTextEdit()
        self.log_edit.setReadOnly(True)
        self.log_edit.setFont(QFont("Consolas", 9)) # 使用等寬字體
        layout.addWidget(self.log_edit)

    @pyqtSlot(str, dict)
    def add_log(self, action_type, params):
        """添加一條模擬指令日誌"""
        now = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        try:
            # 使用 json.dumps 美化字典輸出
            params_str = json.dumps(params, indent=2, ensure_ascii=False)
        except TypeError:
            params_str = str(params) # Fallback for non-serializable items
...
class EFEMSimulationWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        palette.setColor(QPalette.Window, QColor('lightgray')) # 設定背景色
        self.setPalette(palette)
        self.setMinimumHeight(200) # 調整最小高度以適應較小的視窗
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(SIM_LP1_RECT.adjusted(0, 0, 0, -SIM_LP1_RECT.height() + 15), Qt.AlignCenter, "Load Port 1")

        painter.setBrush(QBrush(QColor('orange')))
        painter.drawEllipse(SIM_ROBOT_RECT)
        painter.drawText(SIM_ROBOT_RECT, Qt.AlignCenter, "Robot")

        painter.setBrush(QBrush(QColor('lightgreen')))
        painter.drawRect(SIM_ALIGNER_RECT)
        painter.drawText(SIM_ALIGNER_RECT.adjusted(0, 0, 0, -SIM_ALIGNER_RECT.height() + 15), Qt.AlignCenter, "Aligner")

        painter.setBrush(QBrush(QColor('yellow')))
        painter.drawRect(SIM_STAGE1_RECT)
        painter.drawText(SIM_STAGE1_RECT.adjusted(0, 0, 0, -SIM_STAGE1_RECT.height() + 15), Qt.AlignCenter, "Stage 1")

        painter.setBrush(QBrush(QColor('pink')))
        painter.drawRect(SIM_BUFFER1_RECT)
        painter.drawText(SIM_BUFFER1_RECT.adjusted(0, 0, 0, -SIM_BUFFER1_RECT.height() + 15), Qt.AlignCenter, "Buffer 1")
        ...
        if self.foup_present.get('LP1', False):
            slot_height = (SIM_LP1_RECT.height() - 10) / self.loadport_slots
            max_slots_to_draw = 15
            for i in range(min(self.loadport_slots, max_slots_to_draw)):
                slot_num = self.loadport_slots - i
                slot_key = f'LP1_S{slot_num}'
                if self.wafer_locations.get(slot_key, False):
                    y_pos = SIM_LP1_RECT.top() + 5 + i * slot_height + (slot_height - SIM_WAFER_SIZE) / 2
                    if y_pos + SIM_WAFER_SIZE <= SIM_LP1_RECT.bottom() - 5:
...
class EFemApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("EFEM GUI 控制器 v0.3 (800x600)")
        self.setGeometry(50, 50, 800, 600) # <--- 調整主視窗大小為 800x600
        ...
        # 調整垂直比例以適應 600 的高度
        self.right_splitter.setSizes([250, 350]) # 模擬器佔 250，日誌佔 350

        # --- 將左右面板加入主 Splitter ---
        self.main_splitter.addWidget(self.left_panel)
        self.main_splitter.addWidget(self.right_panel)
        # 調整左右比例以適應 800 的寬度
        self.main_splitter.setSizes([350, 450]) # 控制面板佔 350，右側佔 450
        ...
        # --- 建立並顯示模擬指令視窗 ---
        self.sim_log_window = SimulationLogWindow()
        # 調整模擬指令視窗位置，使其在 800x600 主視窗旁
        self.sim_log_window.setGeometry(self.geometry().right() + 10, self.geometry().top(), 350, 250)
    ...
    def _create_connection_area(self):
        ...
        # 新增按鈕以顯示/隱藏模擬指令視窗
        self.toggle_sim_log_btn = QPushButton("顯示/隱藏模擬指令")
        self.toggle_sim_log_btn.setCheckable(True) # 設為可選中狀態
        self.toggle_sim_log_btn.toggled.connect(self.toggle_simulation_log_window)
        layout.addWidget(self.toggle_sim_log_btn, 4, 0, 1, 2)
    ...
    def _create_simulation_area(self):
        """建立 EFEM 模擬器顯示區域"""
        self.simulation_group = QGroupBox("EFEM 模擬器")
        layout = QVBoxLayout()
        self.simulation_widget = EFEMSimulationWidget() # <--- 建立模擬器 Widget
    ...
    def handle_visual_update(self, action_type, params):
        """處理來自流程執行緒的模擬器更新請求"""
        # self.log_message(f"模擬器更新: {action_type} - {params}", "gray") # 改為顯示在獨立視窗
        if self.sim_log_window:
            self.sim_log_window.add_log(action_type, params) # <--- 更新獨立視窗
        # 仍然更新圖形模擬器
        self.simulation_widget.update_simulation(action_type, params)

    @pyqtSlot(bool)
    def toggle_simulation_log_window(self, checked):
        """顯示或隱藏模擬指令視窗"""
        if self.sim_log_window:
            if checked:
                self.sim_log_window.show()
                self.sim_log_window.raise_() # 提到最前
                self.sim_log_window.activateWindow() # 激活
            else:
                self.sim_log_window.hide()
    ...
    def closeEvent(self, event):
        ...
        # 關閉模擬指令視窗 (這次是真的關閉)
        if self.sim_log_window:
            # 斷開 closeEvent 的連接，以允許其被銷毀
            try:
                self.sim_log_window.closeEvent = lambda e: QWidget.closeEvent(self.sim_log_window, e)
            except Exception:
                pass # 可能已經關閉
            self.sim_log_window.close()
...
if __name__ == '__main__':
    ...
    mainWin.show()
    # 顯示模擬指令視窗 (如果需要預設顯示)
    if mainWin.sim_log_window:
        # mainWin.sim_log_window.show() # 預設不顯示，由按鈕控制
        mainWin.toggle_sim_log_btn.setChecked(False) # 同步按鈕狀態
