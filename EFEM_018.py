...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                             ...
                             QMessageBox, QComboBox, QTabWidget, QSplitter, QFrame, QScrollArea) # QScrollArea 稍後移除
...
AL_X, AL_Y = RBT_X + RBT_S + 25, 20 # Aligner 移到更上方
STG_X, STG_Y = AL_X, LP1_Y + LP_H - STG_S # Stage 放在 Aligner 下方，與 LP 對齊底部
BUF_X, BUF_Y = AL_X + AL_S + 25, RBT_Y # Buffer 放在 Aligner 右側
...
SIM_WAFER_SIZE = 8 # Wafer 尺寸也稍微縮小
# 計算模擬器需要的最小尺寸 (根據元件最右和最下邊緣)
...
class EFEMSimulationWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(SIM_LP1_RECT.adjusted(0, 5, 0, 0), Qt.AlignHCenter | Qt.AlignTop, "LP1") # 標籤移到頂部
        ...
        painter.drawText(SIM_ALIGNER_RECT.adjusted(0, 5, 0, 0), Qt.AlignHCenter | Qt.AlignTop, "Aligner")

        painter.setBrush(QBrush(QColor('yellow')))
        painter.drawRect(SIM_STAGE1_RECT)
        painter.drawText(SIM_STAGE1_RECT.adjusted(0, 5, 0, 0), Qt.AlignHCenter | Qt.AlignTop, "Stage1")

        painter.setBrush(QBrush(QColor('pink')))
        painter.drawRect(SIM_BUFFER1_RECT)
        painter.drawText(SIM_BUFFER1_RECT.adjusted(0, 5, 0, 0), Qt.AlignHCenter | Qt.AlignTop, "Buffer1")
        ...
        if self.foup_present.get('LP1', False):
            # 繪製 Load Port 內的 Wafer (更緊湊)
            slot_draw_height = SIM_LP1_RECT.height() - 20 # 上下留白
            slot_y_start = SIM_LP1_RECT.top() + 10
            max_slots_to_draw = 10 # 最多畫10個示意
            slot_display_height = slot_draw_height / max_slots_to_draw # 計算每個顯示 slot 的高度

            for i in range(max_slots_to_draw):
                # 模擬顯示前10個 slot 的狀態 (假設 mapping 結果是從 slot 25->1)
                slot_num_actual = self.loadport_slots - i # 實際 slot 編號 (25, 24, ...)
