...
from tkinter import ttk, scrolledtext, messagebox, simpledialog, Listbox, END, SINGLE, EXTENDED, ANCHOR
import socket
import threading
import time
import queue
...
# ... (connect_efem, disconnect_efem, send_command, receive_data, process_received_message 函數保持不變) ...
...
class EfemApp:
    def __init__(self, root):
        self.root = root
        self.root.title("EFEM 測試程式 v1.2 (序列建構)")
        self.root.geometry("900x700") # 再次加大視窗
        ...
        self._create_sequence_tab() # 修改此函數
        ...
        self.log_text = scrolledtext.ScrolledText(log_frame, wrap=tk.WORD, height=10, width=100) # 加寬
    ...
    def _create_efem_tab(self):
        ...
        # 修改按鈕 command，使其加入序列
        ttk.Button(control_frame, text="加入 Home EFEM", command=lambda: self.add_command_to_sequence("Home,EFEM")).grid(row=0, column=0, padx=5, pady=5)
        ttk.Button(control_frame, text="加入 GetStatus EFEM", command=lambda: self.add_command_to_sequence("GetStatus,EFEM")).grid(row=0, column=1, padx=5, pady=5)
        ttk.Button(control_frame, text="加入 GetVersion EFEM", command=lambda: self.add_command_to_sequence("GetVersion,EFEM")).grid(row=0, column=2, padx=5, pady=5)

        mode_frame = ttk.LabelFrame(frame, text="模式切換")
        mode_frame.pack(pady=10, padx=10, fill="x")
        ttk.Button(mode_frame, text="加入 Remote 模式", command=lambda: self.add_command_to_sequence("Remote,EFEM")).grid(row=0, column=0, padx=5, pady=5)
        ttk.Button(mode_frame, text="加入 Local 模式", command=lambda: self.add_command_to_sequence("Local,EFEM")).grid(row=0, column=1, padx=5, pady=5)

        tower_frame = ttk.LabelFrame(frame, text="塔燈/蜂鳴器")
        tower_frame.pack(pady=10, padx=10, fill="x")
        ttk.Button(tower_frame, text="加入 塔燈 (紅閃)", command=lambda: self.add_command_to_sequence("SignalTower,EFEM,Red,Flash")).grid(row=0, column=0, padx=5, pady=5)
        ttk.Button(tower_frame, text="加入 塔燈 (綠亮)", command=lambda: self.add_command_to_sequence("SignalTower,EFEM,Green,On")).grid(row=0, column=1, padx=5, pady=5)
        ttk.Button(tower_frame, text="加入 塔燈 (全滅)", command=lambda: self.add_command_to_sequence("SignalTower,EFEM,All,Off")).grid(row=0, column=2, padx=5, pady=5)
        ttk.Button(tower_frame, text="加入 蜂鳴器 (On)", command=lambda: self.add_command_to_sequence("SetBuzzer,EFEM,1,On")).grid(row=1, column=0, padx=5, pady=5) # 假設蜂鳴器編號為 1
        ttk.Button(tower_frame, text="加入 蜂鳴器 (Off)", command=lambda: self.add_command_to_sequence("SetBuzzer,EFEM,1,Off")).grid(row=1, column=1, padx=5, pady=5)


    def _create_loadport_tab(self):
        """建立 Load Port 頁籤的內容 (使用 Combobox 加入序列)"""
        ...
        action_frame = ttk.LabelFrame(frame, text="選擇動作加入序列")
        ...
        lp_actions = [
            ...
            "GotoSlot", "Home", "GetStatus", "ResetError", "GetMapResult", "ReadFoupID", "GetCurrentLPWaferSize"
        ]
        ttk.Label(action_frame, text="選擇 Load Port 動作:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.lp_action_combo = ttk.Combobox(action_frame, textvariable=self.lp_action_var, values=lp_actions, width=25, state="readonly") # 加寬
        self.lp_action_combo.grid(row=0, column=1, padx=5, pady=5, sticky="w")
        if lp_actions: self.lp_action_combo.current(0)

        ttk.Button(action_frame, text="加入序列", command=self.add_loadport_action_to_sequence).grid(row=0, column=2, padx=10, pady=5)


    def _create_aligner_tab(self):
        """建立 Aligner 頁籤的內容 (使用 Combobox 加入序列)"""
        ...
        self.al_degree_entry.insert(0, "900")
        ...
        al_actions = [
            "Home", "GetStatus", "ResetError", "CheckWaferPresence",
            "Alignment", "Vacuum On", "Vacuum Off", "Clamp", "Unclamp",
            "MoveToLoadPosition", "SetAlignmentAngle", "MoveRelativeAngle",
            "SetWaferType", "SetWaferMode", "SetWaferSize", "SetSpeed" # 需要彈窗的指令
        ]
        ttk.Label(action_frame, text="選擇 Aligner 動作:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.al_action_combo = ttk.Combobox(action_frame, textvariable=self.al_action_var, values=al_actions, width=25, state="readonly") # 加寬
        self.al_action_combo.grid(row=0, column=1, padx=5, pady=5, sticky="w")
        if al_actions: self.al_action_combo.current(0)

        ttk.Button(action_frame, text="加入序列", command=self.add_aligner_action_to_sequence).grid(row=0, column=2, padx=10, pady=5)


    def _create_robot_tab(self):
        """建立 Robot 頁籤的內容 (使用 Combobox 加入序列)"""
        ...
        self.rb_dest_entry.insert(0, "Loadport1")
        ...
        rb_actions = [
            "Home", "Stop", "GetStatus", "CheckWaferPresence", "GetForkInfo", "GetForkStatus", "GetErrorCode", "GetVersion", # 簡單指令
            "SmartGet", "SmartPut", "GetStandby", "PutStandby", "DoubleGet", "DoublePut", # Arm, Dest, Slot
            "TwoStepGet", "TwoStepPut", "GetStep", "PutStep", "MultiGet", "MultiPut", # Arm, Dest, Slot + (Step/Forks)
            "MoveToStation", # Arm, Dest
            "Vacuum On", "Vacuum Off", "EdgeGrip On", "EdgeGrip Off", # Arm, SW
            "FlipWafer Front", "FlipWafer Back", "GetFlipDirection" # Arm, (Direction)
        ]
        ttk.Label(action_frame, text="選擇 Robot 動作:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.rb_action_combo = ttk.Combobox(action_frame, textvariable=self.rb_action_var, values=rb_actions, width=25, state="readonly") # 加寬
        self.rb_action_combo.grid(row=0, column=1, padx=5, pady=5, sticky="w")
        if rb_actions: self.rb_action_combo.current(0)

        ttk.Button(action_frame, text="加入序列", command=self.add_robot_action_to_sequence).grid(row=0, column=2, padx=10, pady=5)


    def _create_sequence_tab(self):
        """建立指令序列頁籤的內容 (使用 Listbox 管理)"""
        ...
        ttk.Label(left_frame, text="指令序列:").pack(anchor="w")

        listbox_frame = ttk.Frame(left_frame)
        listbox_frame.pack(fill=tk.BOTH, expand=True)

        self.sequence_listbox = Listbox(listbox_frame, height=20, width=60, selectmode=EXTENDED) # 允許多選
        ...
        self.sequence_listbox.config(yscrollcommand=listbox_scrollbar.set)

        # 右側：操作按鈕
        right_frame = ttk.Frame(frame)
        right_frame.pack(side=tk.LEFT, padx=10, pady=10, fill=tk.Y)

        ttk.Button(right_frame, text="↑ 上移", command=self.move_item_up).pack(pady=3, fill=tk.X)
        ttk.Button(right_frame, text="↓ 下移", command=self.move_item_down).pack(pady=3, fill=tk.X)
        ttk.Button(right_frame, text="刪除選定", command=self.delete_selected_from_sequence).pack(pady=3, fill=tk.X)
        ttk.Button(right_frame, text="加入 Wait", command=self.add_wait_to_sequence).pack(pady=10, fill=tk.X)
        ttk.Button(right_frame, text="清除全部", command=self.clear_sequence).pack(pady=3, fill=tk.X)
        ttk.Separator(right_frame, orient=tk.HORIZONTAL).pack(pady=10, fill=tk.X)
        ttk.Button(right_frame, text="執行序列", command=self.run_sequence).pack(pady=10, fill=tk.X)
        ...
        for cmd in example_sequence:
            self.sequence_listbox.insert(END, cmd)


    # --- 序列管理函數 ---
    def add_command_to_sequence(self, command_string):
        """將指令字串加入到序列 Listbox"""
        if not command_string:
            return
        self.sequence_listbox.insert(END, command_string)
        self.sequence_listbox.see(END) # 滾動到最後
        self.log_message(f"已加入序列: {command_string}\n")
        # 切換到序列頁籤方便查看
        self.notebook.select(self.sequence_tab)
    ...
    def add_wait_to_sequence(self):
        """彈出對話框詢問秒數，並加入 Wait 指令"""
        seconds = simpledialog.askfloat("加入 Wait", "請輸入等待秒數:", parent=self.root, minvalue=0.1)
        if seconds is not None:
            command = f"Wait,{seconds}"
            self.add_command_to_sequence(command)

    def clear_sequence(self):
        """清除序列 Listbox 中的所有項目"""
        if messagebox.askokcancel("確認", "確定要清除序列中的所有指令嗎？"):
            self.sequence_listbox.delete(0, END)
            self.log_message("已清除所有序列指令。\n")
    ...
    def run_sequence(self):
        """執行指令序列 Listbox 中的指令"""
        commands = self.sequence_listbox.get(0, END) # 從 Listbox 獲取所有指令
        if not commands:
        ...
        # 在新執行緒中執行，避免阻塞 GUI
        threading.Thread(target=self._execute_sequence_thread, args=(list(commands),), daemon=True).start() # 轉換為 list
    ...
    def highlight_sequence_line(self, index):
         ...
         if hasattr(self, 'sequence_listbox') and self.sequence_listbox.winfo_exists():
             ...
             for i in range(self.sequence_listbox.size()):
                 self.sequence_listbox.itemconfig(i, bg='white', fg='black')
             # 高亮當前行
             if index >= 0 and index < self.sequence_listbox.size():
                 self.sequence_listbox.itemconfig(index, bg='lightblue', fg='black')
                 self.sequence_listbox.see(index) # 確保該行可見
    ...
    def add_loadport_action_to_sequence(self):
        ...
        if not selected_action or not lp_num:
            self.log_message("錯誤：請選擇動作並輸入 Loadport 編號。\n")
            return
    ...
    def add_aligner_action_to_sequence(self):
        ...
        if not selected_action or not al_num:
            self.log_message("錯誤：請選擇動作並輸入 Aligner 編號。\n")
            return
        ...
        elif selected_action in ["SetWaferType", "SetWaferMode", "SetWaferSize", "SetSpeed"]:
            # 需要彈窗獲取參數
            prompt_map = {
                "SetWaferType": "輸入類型 (Notch/Flat/Neither):",
                "SetWaferMode": "輸入模式 (Transparent/Nontransparent):",
                "SetWaferSize": "輸入尺寸 (4/6/8/12):",
                "SetSpeed": "輸入速度百分比 (5%~100%):"
            }
            param = simpledialog.askstring("輸入參數", prompt_map[selected_action], parent=self.root)
            if param is not None and param.strip():
                command = f"{selected_action},Aligner{al_num},{param.strip()}"
            elif param is not None:
                 self.log_message(f"錯誤：{selected_action} 指令需要一個有效的參數。\n")
                 return # 取消加入
            else:
                 return # 使用者取消
        else: # 其他簡單指令
    ...
    def add_robot_action_to_sequence(self):
        ...
        if not selected_action_raw or not robot_prefix:
            self.log_message("錯誤：請選擇動作並輸入有效的 Robot 編號。\n")
            return

        command = None
        base_command = selected_action_raw.split(" ")[0]

        # 構建指令 (與 execute_robot_action 類似，但不發送)
