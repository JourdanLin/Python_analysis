...
import tkinter.font as tkFont # 導入字體模組
import socket
import threading
import time
import queue
...
class EfemApp:
    def __init__(self, root):
        self.root = root
        self.root.title("EFEM 測試程式 v1.4 (Calibri 字型)")
        ...
        # print(style.theme_names()) # 查看可用的主題
        # print(style.theme_use()) # 查看當前主題
        try:
            # 確保主題存在，否則 ttk configure 可能報錯
             current_theme = style.theme_use()
             style.configure('.', font=self.app_font) # 設定所有 ttk 元件的預設字型
             ...
             # Combobox 的字型可能需要特殊處理
             # style.map('TCombobox', fieldbackground=[('readonly','white')])
             # style.map('TCombobox', selectbackground=[('readonly', 'blue')])
             # style.map('TCombobox', selectforeground=[('readonly', 'white')])
             style.configure("TCombobox", font=self.app_font) # 嘗試設定 Combobox 字型
        ...
        self.connection_status_label = ttk.Label(top_frame, text="已斷線", foreground="red", font=(self.app_font_family, self.app_font_size, "bold")); self.connection_status_label.pack(side=LEFT, padx=5) # 明確指定狀態標籤字型
        ...
        self.log_text = scrolledtext.ScrolledText(log_frame, wrap=tk.WORD, width=80, height=9, font=self.log_font) # **使用 Calibri 字型**
    ...
    def _create_sequence_panel(self, parent):
        ...
        self.sequence_listbox = Listbox(listbox_frame, height=10, selectmode=EXTENDED, font=(self.app_font_family, self.app_font_size)) # **使用 Calibri 字型**
